...
class LossWrapper(torch.nn.Module):
    ...
    def forward(self, fc_feats, att_feats, labels, masks, att_masks, src1, src2, src3, src4,
                adj1, adj2, adj3, adj4, gts, gt_indices, sc_flag, struc_flag):
        opt = self.opt
        ...
        if struc_flag:
            if opt.structure_loss_weight < 1:
                ...
                lm_loss = self.crit(self.model(fc_feats, att_feats, labels[..., :-1], src1, src2, src3, src4,
                                               adj1, adj2, adj3, adj4, att_masks), labels[..., 1:], masks[..., 1:])
            else:
            ...
            loss = self.crit(self.model(fc_feats, att_feats, labels[..., :-1],  src1, src2, src3, src4,
                                        adj1, adj2, adj3, adj4, att_masks), labels[..., 1:], masks[..., 1:])
        else:
            self.model.eval()
            with torch.no_grad():
